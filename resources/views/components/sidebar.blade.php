...
@if ($links->isNotEmpty())
    <aside {{ $attributes->class(['py-10', $stickyClass]) }}>
        @if (filled($title))
            <p class="text-[11px] font-semibold tracking-[0.1em] text-neutral-500 uppercase">{{ $title }}</p>
        @endif

        <nav
            aria-label="{{ $label }}"
            @class([
                'flex flex-col border-s border-neutral-200',
                'mt-3.5'                                                               => filled($title),
