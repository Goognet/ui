...
@php
    ...
    $posterClass = $ui->classes('poster', 'size-full object-cover object-center transition-[scale] duration-(--duration-slow) ease-(--ease-fluid) group-hover:scale-105 motion-reduce:transition-none motion-reduce:group-hover:scale-100');
@endphp
...
<a
    ...
    data-video
    class="{{ ClassList::merge($ui->classes('base', 'group relative block overflow-hidden rounded-media shadow-control transition-shadow duration-(--duration-base) ease-(--ease-fluid) hover:shadow-control-hover ' . $ratioClass), (string) $attributes->get('class')) }}"
...
>
    ...
    <span
        class="{{ $ui->classes('overlay', 'absolute inset-0 bg-gradient-to-t from-neutral-950/50 via-neutral-950/10 to-neutral-950/25 transition-opacity duration-(--duration-base) ease-(--ease-fluid) group-hover:opacity-80') }}"
        aria-hidden="true"
    ></span>

    <span class="absolute inset-0 grid place-items-center" aria-hidden="true">
        <span class="{{ $ui->classes('play', 'shadow-control-hover grid size-16 place-items-center rounded-full bg-white/95 text-neutral-900 backdrop-blur-sm transition-[scale,background-color] duration-(--duration-base) ease-(--ease-fluid) group-hover:scale-110 group-hover:bg-white group-active:scale-100 motion-reduce:transition-none motion-reduce:group-hover:scale-100 sm:size-20') }}">
    ...
    @if (filled($title))
        <span
            class="{{ $ui->classes('title', 'absolute inset-x-0 bottom-0 p-4 text-sm font-medium text-white sm:p-5 sm:text-base') }}"
            aria-hidden="true"
        >
            {{ $title }}
        </span>
